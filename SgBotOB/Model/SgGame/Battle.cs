        ...
        public BattleLog MakeBattle(Player playerp, Player enemyp)
        {
            ...
            while (fastUnit.Hp > 0 && slowUnit.Hp > 0)
            {
                ...
                if (speedFlag >= 1) // 速度高的攻击
                {
                    ...
                    value += 40;
                    value = SlpzMethods.OutGoodNumber(value, 100, 20);
                    ...
                    critFlag = (int)SlpzMethods.OutGoodNumber(critFlag, 100, 5);
                    #endregion

                    if (SlpzMethods.IsOk(100, critFlag))
                    {
                        isCrit = true;
                    }

                    if (SlpzMethods.IsOk(100, (int)swiftFlag))//判定闪避,生成伤害
                    {
                        isMiss = false;
                        ...
                        phyDmg = (int)(phyDmg * SlpzMethods.MakeRandom(110, 90) / 100);
                        ...
                        magDmg = (int)(magDmg * SlpzMethods.MakeRandom(110, 90) / 100);
                ...
                else // 速度低的攻击
                {
                    ...
                    swiftFlag = SlpzMethods.OutGoodNumber(swiftFlag, 100, 5);
                    #region 暴击处理
                    if (critTemp <= 0)
                    {
                        critTemp = 1;
                    ...
                    critFlag = (int)SlpzMethods.OutGoodNumber(critFlag, 95, 5);
                    #endregion
                    if (SlpzMethods.IsOk(100, critFlag))
                    {
                        isCrit = true;
                    ...
                    //判定闪避,生成伤害
                    if (SlpzMethods.IsOk(100, (int)swiftFlag))
                    {
                        isMiss = false;
